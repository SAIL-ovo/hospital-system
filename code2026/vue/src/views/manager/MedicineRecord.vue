...
// 分页查询医生列表
const load = () => {
  request.get('/medicineRecord/selectPage', {
    params: {
...
// 新增
const add = () => {
  request.post('/medicineRecord/add', data.form).then(res => {
    if (res.code === '200') {
...
// 编辑
const update = () => {
  request.put('/medicineRecord/update', data.form).then(res => {
    if (res.code === '200') {
...
const handleDelete = (id) => {
  ElMessageBox.confirm(
    ...
    request.delete(`/medicineRecord/delete/${id}`).then(res => {
      if (res.code === '200') {
